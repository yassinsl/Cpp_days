...
//remove spaces form bigen and end the string
...
bool valid_value(std::string &string_value, double &value){
  ...
  if(!end || *end ||  value < 0.0) {
    ...
    else if(static_cast<long long>(value) > INT_MAX || static_cast<long long>(value) < INT_MIN) std::cout <<  "Error: too lager a number" << std::endl;
    return false;
  }
  return true;
}
void parse_cvs(map parse_cs)
{
  ...
  while(std::getline(file, line))
  {
    ...
    if(!valid_date(date) && !valid_value(value,value_n))
