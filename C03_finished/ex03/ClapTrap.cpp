...
ClapTrap::ClapTrap(str name) : name(name), HitPoint(10), EnergyPoint(10), AttackDamage(0)
{
...
void ClapTrap::attack(const std::string& target)
{
		...
		else if (this->EnergyPoint <= 0) cout << FIGHTER_MSG << this->name << " has no energy left" << endl;
		else
			cout << FIGHTER_MSG << this->name << " attacks " << target
				<< " causing " + ss.str() + " points of damage! " <<endl, this->EnergyPoint--;
}
...
{	
	...
	if(this->HitPoint <= 0) {cout << FIGHTER_MSG << this->name << " is already destroy and can't repair itself "<<endl;
				return ;}
	if(this->EnergyPoint <= 0) {cout << FIGHTER_MSG << this->name << " has no energy to repair" << endl;
				return;}
	this->HitPoint+= amount, this->EnergyPoint-=1;
	ss2 << this->HitPoint;
